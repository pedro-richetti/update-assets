...
- A automação é feita utilizando GitHub Actions através do arquivo `update.yml`. O fluxo de trabalho é executado automaticamente nos dias úteis às 06:00 (horário de Brasília), roda o script Python para atualizar os dados, realiza o commit e o push das alterações do `dados.json` (caso existam) e, em seguida, faz o deploy da página estática automaticamente no GitHub Pages.